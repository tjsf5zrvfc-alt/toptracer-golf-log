...
SMART GOLFの`portal.sma-gol.app`で利用されているBearer Tokenを保存すると、`GET /api/swing/latest_list`からショットデータを取り込めます。

1. `設定`画面を開きます。
2. `SMART GOLF`の`Bearer Token`にトークンを貼り付けます。
3. `clubType変換`を必要に応じて編集します。
4. `SMART GOLF設定保存`を押します。
5. `記録`画面の`SMART GOLF同期`を押します。
...
- SMART GOLFのBearer TokenはlocalStorageに保存します。共有端末では使わないでください。
